...
| A palette or type decision that lives only in the chat | Re-litigated every quarter, and the third designer picks a fourth grey | Record the decision and rejected alternative in `<state_root>/data/designer/memory.md` using `references/memory-template.md`. |
