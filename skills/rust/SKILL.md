...
name: rust
description: >
  Write, debug, and optimize Rust code, crates, and Cargo builds. Use when the
  borrow checker rejects code, rustc reports moves/lifetimes/trait bounds, async
  tasks deadlock or lose data across await, Cargo builds are slow or diverge in
  CI, or when working with unsafe, FFI, proc macros, no_std, wasm, or cross-
  compilation. Load files from `references/` for deeper constraints.
metadata:
  version: "1.1.0"
  openclaw: '{"emoji":"🦀","displayName":"Rust","requires":{"bins":["rustc","cargo"]}}'
  related-skills: '{"cpp":"when the other side of the FFI boundary is C++, or when comparing ownership models","go":"the other systems language on the same team; different concurrency tradeoffs"}'
---

User preferences and memory live in `<state_root>/rust/` (see `references/setup.md` on first use, `assets/memory-template.md` for the file format). If you have data at an old location, move it to `<state_root>/rust/`, and say in one line that you moved it and from where.

## When to load

- Load `references/ownership.md` when diagnosing borrow checker moves or mutable alias rejections.
- Load `references/lifetimes.md` when functions return references or throw lifetime specifier errors.
- Load `references/async.md` for Tokio/async runtime deadlocks, `select!`, or spawned tasks.
- Load `references/cargo.md` for slow builds, dependency collisions, and cross-compilation.
- Load `references/unsafe.md` when interacting with C FFI or diagnosing segfaults.
...
| Two mutable borrows refused on code that looks fine | The checker is field-sensitive on direct field access and field-insensitive through a method call — destructure or `split_at_mut` → `references/ownership.md` |
| A returned reference "does not live long enough" | Nothing borrowed from a frame outlives it: return owned data, or an index → `references/lifetimes.md` |
| `missing lifetime specifier` on a signature | Two input references and no elision rule applies — name which one the output borrows from → `references/lifetimes.md` |
| "trait bound not satisfied" on a type that clearly implements it | Missing `use` for the trait, or `&T` supplied where `T` was required — read the `required by` chain bottom-up → `references/traits.md` |
| "future cannot be sent between threads safely" | A `!Send` value (`std` `MutexGuard`, `Rc`, `RefCell` ref) is alive across an `.await` → `references/async.md` |
| Async program stalls with no error and no progress | A blocking call is occupying a runtime worker → `references/async.md` |
| Program deadlocks | One thread locks the same lock twice, or two threads take two locks in opposite order → `references/concurrency.md` |
| `already borrowed: BorrowMutError` at runtime | Overlapping `RefCell` borrows — the check you moved from compile time to runtime → `references/ownership.md` |
| `String` vs `&str`, a panic on a string slice, UTF-8, or path handling | Byte indices are not characters and paths are not UTF-8 → `references/strings-and-text.md` |
| A `match` is unreadable, non-exhaustive, or the type allows a state you never want | Model it as an enum, then let exhaustiveness find the sites → `references/pattern-matching.md` |
| Nothing in the logs explains what the service was doing | No subscriber, or context in strings instead of span fields → `references/observability.md` |
| "Where does this type go, and what should be public" | Module tree, `pub(crate)`, re-exports, lib vs bin split → `references/modules-and-layout.md` |
| Error names two identical-looking types (`expected Uri, found Uri`) | Semver-incompatible duplicate of one crate in the graph: `cargo tree -d` → `references/cargo.md` |
| Builds locally, fails in CI | Toolchain, lockfile, or feature-unification drift → `references/cargo.md` |
| Build is slow | `cargo build --timings` first; linker and codegen settings before restructuring code → `references/cargo.md` |
| Program is slow | Profile a release build with symbols; allocation and `clone` before algorithms → `references/performance.md` |
| Segfault, corrupted data, or nondeterministic "works on my machine" | Only reachable through `unsafe` or FFI — run it under Miri → `references/unsafe.md` |
| Anything else | Read the whole rustc output bottom-up including `note:` and `help:`, then `rustc --explain E0xxx` → `references/compiler-errors.md` |

Depth on demand: `references/compiler-errors.md` error code → real cause → first move · `references/ownership.md` moves, borrows, interior mutability · `references/lifetimes.md` elision, variance, `'static`, HRTB · `references/traits.md` coherence, generics vs `dyn`, associated types · `references/pattern-matching.md` `match` ergonomics, `let else`, exhaustiveness, `Option`/`Result` combinators, modeling states · `references/errors.md` `Result` design, `?`, panics, backtraces · `references/async.md` runtimes, `Send` bounds, cancellation · `references/concurrency.md` threads, locks, channels, atomics · `references/strings-and-text.md` `String`/`&str`, UTF-8, formatting, `Cow`, paths · `references/collections.md` `Vec`, maps, sets, queues, iterators · `references/performance.md` profiling, allocation, build settings · `references/observability.md` `tracing`, spans, metrics, panic hooks · `references/cargo.md` features, workspaces, lockfiles, build times · `references/modules-and-layout.md` module tree, visibility, lib/bin/tests layout, when to split a crate · `references/testing.md` layout, doctests, property tests, Miri, fuzzing · `references/unsafe.md` UB rules, raw pointers, safety comments · `references/ffi.md` C interop in both directions · `references/macros.md` `macro_rules!` and proc macros · `references/api-design.md` public APIs and semver hazards · `references/serde.md` derive traps and format edge cases · `references/embedded-no-std.md` firmware and `no_std` · `references/wasm.md` browser and WASI targets · `references/cross-compilation.md` target triples, musl, linkers, containers.
...
5. **One error type per boundary: enum outward, dynamic inward.** A library returning `anyhow::Error` denies callers the ability to match on the failure; a binary with 40 enum variants pays for matching nobody does. Default: a `thiserror` enum in library crates, `anyhow::Result` with `.context()` in binaries (`references/errors.md`).
...
| E0106 | Missing lifetime specifier | Elision cannot choose among multiple inputs — name it (`references/lifetimes.md`) |
| E0308 | Mismatched types | Compare the two full paths in the message: usually `&T` vs `T`, or two versions of one crate (`references/cargo.md`) |
| E0277 | Trait bound not satisfied | Missing `use` of the trait, or a bound to add — read `required by` bottom-up |
| E0038 | Trait is not `dyn` compatible (formerly "object safe") | Generic method, `Self` return, or associated const — use generics or split the trait (`references/traits.md`) |
...
| E0117 / E0119 | Orphan rule / conflicting impls | Newtype wrapper; either the trait or the type must be local (`references/traits.md`) |
...
| Anything else | Every code is stable and documented | `rustc --explain E0xxx`, then `references/compiler-errors.md` for the symptoms that have no code |
...
| Borrow when possible, allocate only on modification | `Cow<'a, str>` | Removes "clone just in case" from hot paths (`references/strings-and-text.md`) |
...
| A counter or a flag | `AtomicUsize`, `AtomicBool` | Cheapest sharing available; memory ordering is a real decision (`references/concurrency.md`) |
...
8. **`unsafe`.** Only with a `// SAFETY:` comment naming the invariant, and Miri in CI (`references/unsafe.md`).
...
- Public enums and structs that may grow carry `#[non_exhaustive]` (`references/api-design.md`)
...
User-dependent variables. Defaults apply until the user states a preference; store them in `<state_root>/rust/config.yaml`.
...
| async_runtime | tokio \| async-std \| smol \| embassy \| none | tokio | Chooses the runtime API in every async example and the spawn, timer, and IO types used from `references/async.md` |
| error_style | thiserror \| anyhow \| std-enum \| box-dyn \| auto | auto (thiserror in libraries, anyhow in binaries) | Sets the error type generated at each boundary and the `?` conversion strategy in `references/errors.md` |
...
- **Tooling**: `cargo test` vs `cargo nextest`, criterion vs a custom harness, `cargo` vs `just`/`make` wrappers, linker choice (`lld`, `mold`) — affects the commands in `references/testing.md` and `references/performance.md`
- **Conventions**: module layout (`mod.rs` vs `name.rs`), builder vs plain constructors, re-export policy at the crate root, doc-comment strictness — affects `references/modules-and-layout.md` and `references/api-design.md`
- **Platform**: default target triple, libc flavor (gnu vs musl), hosted vs `no_std`, wasm target — affects `references/cross-compilation.md`, `references/embedded-no-std.md`, `references/wasm.md`
...
- **Registries and sources**: crates.io vs a private or mirrored registry, `cargo vendor` plus source replacement, air-gapped or offline builds, git vs path dependencies for internal crates — affects the manifest, `.cargo/config.toml`, and every install command in `references/cargo.md`
- **Work order and gates**: `cargo check` in the edit loop vs full builds, whether clippy runs pre-commit or only in CI, whether `cargo semver-checks` gates release PRs, test-before-fix discipline — affects the command sequences in `references/performance.md`, `references/testing.md`, and `references/api-design.md`
...
- **Cadence**: dependency update and audit frequency, MSRV bump policy — affects the maintenance advice in `references/cargo.md`
...
| `mem::transmute` to change a lifetime | Erases the exact property the compiler was checking; UB the moment the data dies | Restructure, or a raw pointer plus a SAFETY comment (`references/unsafe.md`) |
| `async fn` on functions that never `.await` | Colors the whole call graph and wraps sync work in a state machine | `async` only where an await point exists (`references/async.md`) |
...
| `cargo clean` to fix a slow build | Deletes the incremental cache and guarantees the slowest possible next build | `cargo build --timings` and read the graph (`references/cargo.md`) |
...
| `unwrap()` on `Mutex::lock()` everywhere by reflex | Right most of the time, but hides that poisoning means another thread panicked mid-invariant | Handle or document the poison case (`references/concurrency.md`) |
| Adding lifetime parameters until it compiles | Produces signatures nobody can call and hides the real design error | Return owned data first; add lifetimes once you can state what borrows from what (`references/lifetimes.md`) |
