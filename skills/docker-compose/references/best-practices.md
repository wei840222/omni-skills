...
healthcheck:
  ...
  start_period: 30s
```
- `start_period`: initial grace period—health failures are ignored during this time
...
- Copy limits to production config to ensure consistent constraints in production
...
services:
  mailhog:
    profiles: [dev]
```
- Services with profiles remain inactive by default—cleaner `docker compose up`
...
## Docker Compose Watch (V2+)

- Use `docker compose watch` for live-reloading code instead of generic bind mounts where possible.
- Defines syncing rules:
```yaml
services:
  web:
    build: .
    develop:
      watch:
        ...
        - action: rebuild
          path: package.json
```
- Action `sync` directly updates files; action `rebuild` rebuilds the image; action `sync+restart` syncs and restarts the container.
- Improves development speed by not requiring full rebuilds or heavy host-container mounts.

## Docker Compose V2 Command differences

- Docker Compose V1 (`docker-compose`) is deprecated. Use Docker Compose V2 (`docker compose`, no hyphen).
- Use exclusively Docker Compose V2 commands in instructions.
