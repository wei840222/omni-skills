# Domain Knowledge and Research

## Drafting Velocity vs. Editing
- **Source**: "Writing Down the Bones" by Natalie Goldberg (proxy concept: separating creator from editor).
- **URL**: https://en.wikipedia.org/wiki/Writing_Down_the_Bones
- **Concept**: The chapter loop explicitly separates first-drafting from micro-editing to prevent momentum loss.

## Book Architecture and Promise
- **Source**: The "Save the Cat" beats applied to nonfiction/fiction. 
- **URL**: https://en.wikipedia.org/wiki/Save_the_Cat!_The_Last_Book_on_Screenwriting_You%27ll_Ever_Need
- **Concept**: Locking the book promise before drafting ensures the structural backbone supports a clear transformation.

## Pacing and Cognitive Load
- **Source**: Cognitive Load Theory in instructional design.
- **URL**: https://en.wikipedia.org/wiki/Cognitive_load
- **Concept**: Structuring chapters by discrete reader outcomes rather than word count.
