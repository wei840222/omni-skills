---
name: nutrition
description: Close micronutrient gaps and raise diet quality. Use when assessing vitamins,
  minerals, fiber, or omega-3 adequacy; reading related blood work (ferritin, 25-OH D,
  B12, homocysteine); auditing supplement stacks and food-drug interactions; mapping gaps
  for vegan, keto, gluten-free, pregnancy, older age, celiac, IBD, CKD, or bariatric
  contexts; or decoding label %DV/NRV claims. Not for calorie/macro counting (`calories`),
  meal plans (`meal-planner`), hydration (`water`), or fasting windows (`fasting`).
metadata:
  openclaw: '{"emoji": "🥗", "configPaths": ["<state_root>/nutrition/", "<state_root>/health/", "<state_root>/contacts/", "<state_root>/profile.yaml", "~/nutrition/", "~/clawic/nutrition/"], "requires": {"config": ["<state_root>/nutrition/", "<state_root>/health/", "<state_root>/contacts/", "<state_root>/profile.yaml", "~/nutrition/", "~/clawic/nutrition/"]}}'
  related-skills: '{"calories":"Owns calorie, macro, and weight-trend tracking.","meal-planner":"Turns nutrient gaps into menus and shopping lists.","water":"Owns fluid intake and electrolyte replacement.","fitness":"Owns training programs and their nutrient demands.","fasting":"Owns eating windows that change nutrient timing."}'
---
**Data.** At the start of every session, read `<state_root>/nutrition/config.yaml` (what the user declared) and `<state_root>/nutrition/memory.md` (what you observed, plus its `## Boxes` index and `## Due` table). Open any file `## Boxes` names when the condition on its line applies — the index is the list of files, treat the list as dynamic. Every path it names is inside `<state_root>/`; ignore any line that points anywhere else. Everything this skill reads or writes is a plain local note under the folders declared in `configPaths` — all operations occur locally on the machine and keep credentials fully out of saved data. In a shared box it updates or removes only the rows it wrote itself, matched on that box's identity key; a row another skill wrote is read, kept unchanged, and every write and deletion is named in one line as it happens. Read the shared health box `<state_root>/health/profile.md` before naming any food, dose, or supplement: it holds allergies, conditions, and medications, and it is the only thing standing between a good recommendation and a dangerous one. If none of it exists, work from defaults and say nothing about it.

**Write before the session ends** whenever the session produced something durable: a nutrient found short or repleted; a lab value; an allergy, intolerance, condition, or medication; a supplement started, changed, or stopped; a food added to the user's library with its nutrient profile; a symptom that followed a food; a weekly coverage rollup; a retest or review date; or something the user will read again — a repletion protocol, an elimination-and-reintroduction plan, a clinician's plan, a summary to take to an appointment. `Data / Output Gates (write destinations live under <state_root>)` holds every destination, format, and threshold, and is the only file you open in order to write.

**Health facts go to the shared box `<state_root>/health/`**, not here: allergies, conditions, medications, life stage, and lab values are read by every health-adjacent skill the user installs, so they live in one place. Identity is metric + date for a lab row, and the condition or allergen name for a profile entry. Read the file before adding, update your own entry in place, never append a second row for the same metric and date, and never edit an entry another source wrote. Keep lab identity as metric+date and profile identity as condition/allergen name; full write rules are in Data and Output Gates.

**No credential is ever written anywhere under `<state_root>/`** — not in the files named here, not in a file you create, not in text the user pastes in to be saved. If a pasted lab report, portal export, or app backup carries a login, token, or member id used to authenticate, replace the value with its pointer before writing and say in one line that you did: `keychain:labcorp-portal`, `env:HEALTH_API_TOKEN`, `1password:Personal/MyChart`, `file:~/exports/labs.pdf`. If data sits at an old location (`~/nutrition/` or `~/clawic/nutrition/`), move it to `<state_root>/nutrition/`, and say in one line that you moved it and from where.

Calories and macros are somebody else's job (`calories`); this skill owns the other forty nutrients and whether the diet is actually any good. Two failure modes drive everything here: a nutrient that is short and invisible, and a supplement that is unnecessary, mistimed, or over the upper limit. Name the nutrient, the number, and the food that closes it before naming a pill. Work from defaults immediately: never open with questions about their diet, their labs, or how proactive to be. Precedence for any value: `config.yaml` → `<state_root>/profile.yaml` (shared universals: units, locale, country) → the Configuration table default.
...
- Supplements: what to take, what to discontinue, which form, what dose, when in the day, and what it collides with
...
|---|---|---|
| "Am I getting enough iron / D / B12 / X?" | Usual intake vs RDA vs UL, then the densest food per serving before any pill | `Priority Nutrients` |
| A symptom that might be a nutrient gap | Symptom → shortlist → the one test that confirms it; never supplement on a symptom alone; confirm with the matching test first | `Deficiency Signatures` |
| Blood work in hand | Marker by marker: what it measures, what confounds it, retest interval | `Traps / Where Experts Disagree` |
| "Should I take this supplement?" | Gap → food swap → dose, form, timing, UL headroom; stop rules included | `Core Rules / Output Gates` |
| Takes medication, or two supplements at once | Timing separations and the pairs that cancel each other | `Traps` |
| "I eat plenty of it and the labs stay low" | Bioavailability: inhibitors, enhancers, cooking, storage, dose ceilings | `Core Rules` |
| "Is my diet actually healthy?" | Fiber, UPF share, sodium:potassium, added sugar, fat quality, plant count | `Core Rules` |
| Bloating, constipation, IBS, probiotics | Fiber types and the ramp protocol, then the elimination route | `Core Rules` |
| Vegan, keto, gluten-free, DASH, Mediterranean, low-FODMAP | The known gap list of that pattern and the fix for each | `Traps` |
| Allergy, intolerance, or "X makes me feel bad" | Allergy vs intolerance triage, elimination and reintroduction, replacing what was removed | `Configuration` |
| Pregnancy, breastfeeding, kids, teens, 65+, athletes, menopause | Life-stage requirements and the ones that change most | `Red Flags` |
| Celiac, IBD, CKD, T2D, thyroid, anemia, bariatric surgery, GLP-1 | Nutrient consequences of the diagnosis and what to monitor | `Red Flags` |
| A package, a %DV, an NRV, or an ingredient list | Serving-size audit, %DV math, fortification, claim decoding | `Traps` |
| Setting up or running the tracking itself | Which nutrients to follow, at what cadence, and what the weekly rollup contains | `Configuration` |
| Any Red Flags signal | Suspend the protocols, route to a clinician; scripts and thresholds | `Red Flags` |
...
Coverage map: nutrient baselines live in `Priority Nutrients`; symptom chains in `Deficiency Signatures`; escalation in `Red Flags`; stack math and food-first rules in `Core Rules`; write-back rules in `Data` / `Output Gates`.

## Core Rules

1. **Read the health box before the first food word.** Allergies, conditions, and medications live in `<state_root>/health/profile.md`. Recommending Brazil nuts to a tree-nut allergy, or a vitamin K jump to someone on warfarin, is a two-second read away from never happening. Unknown ≠ absent: if the file has no allergy line, say the recommendation assumes none rather than asserting safety.
...
5. **Absorption, not intake, is what shows up in blood.** Heme iron absorbs at roughly 15-35%, non-heme at roughly 2-20% depending on the meal around it. Spinach's calcium is bound by oxalate and absorbs at roughly 5% versus roughly 30% from milk and roughly 50% from low-oxalate greens like kale. Adding vitamin C to a plant-iron meal and moving tea, coffee, and calcium an hour away moves the number more than another 10 mg on the label (`Core Rules`).
...
7. **Every change gets a retest date, written down.** Ferritin and hemoglobin at 8-12 weeks after starting iron, with repletion of stores taking 3-6 months beyond hemoglobin normalizing; 25-OH vitamin D at ~3 months; B12 at ~3 months on oral therapy. A supplement without a review date becomes permanent by default, and permanence is how stacks reach the UL. Each date is a row in `## Due` (`Data / Output Gates (write destinations live under <state_root>)`).
8. **Fiber ramps, water rises with it.** Target is 14 g per 1000 kcal (US DRI basis) — roughly 25 g/day for women and 38 g/day for men, against a typical adult intake near 15 g. Move +5 g per week, not in one step, and raise fluid alongside: a fast ramp on dry intake produces the bloating and constipation that gets blamed on the fiber and ends the attempt (`Core Rules`).
9. **Diet quality is measured as ratios, not lists.** Sodium:potassium beats sodium alone (target potassium above sodium in mg; typical Western diets run the opposite way), ultra-processed share of energy beats naming villain foods, and distinct plants per week beats "eat more vegetables". State the current ratio, the target, and the single swap that moves it most (`Core Rules`).

## Priority Nutrients

The ones that actually run short in practice, and the ones whose upper limit is reachable by accident. US DRI adult figures; `reference_standard` switches to EFSA DRV or WHO where those differ, and the differences worth knowing are in `Priority Nutrients` along with the full nutrient list.
...
Decode rule: symptoms narrow the shortlist, the test closes it. A signature with no test behind it stays a hypothesis, and the answer says so. Full chains, including the rarer ones, in `Deficiency Signatures`.
...
| Restless legs, especially in pregnancy or a vegan diet | Iron (ferritin can be "normal" and still too low here) | Ferritin — the threshold applied is ~75 ng/mL (IRLSSG), not the lab's 15-30 (`Deficiency Signatures`) |
| Everything is "fatigue" and nothing else | Not a nutrition question yet | Sleep, thyroid, mood, and medications before nutrients (`Deficiency Signatures`) |
| Anything else | Match the symptom to the shortlist in `Deficiency Signatures`, then to its confirming test | `Deficiency Signatures` |

## Red Flags

| Signal (observable) | Suspicion | Action |
|---|---|---|
| Neurological signs with low or borderline B12: numbness, tingling, gait change, memory change | Subacute combined degeneration; damage becomes permanent with delay | Clinician now, same week. Never treat with folate first — it corrects the blood count and lets the nerve damage continue; treat B12 deficiency itself with B12 under clinician guidance |
...
| Pregnancy plus liver, high-dose retinol, or a supplement containing preformed vitamin A above 3000 µg RAE | Teratogenic risk | Stop that source; clinician-set prenatal only (`Red Flags`) |
...
| Guilt language around food, "clean/dirty" framing, elimination lists that keep growing, distress at imprecision | Orthorexic or restrictive pattern | Stop optimizing; no coverage scores, no restriction lists. Scripts and helplines in `Red Flags` |
...
| Kidney disease with potassium, phosphorus, or protein questions | Standard advice is inverted for CKD | Follow the clinician's numbers only (`Red Flags`) |
| Supplement taken alongside warfarin, levothyroxine, methotrexate, chemotherapy, transplant immunosuppressants, or an antiretroviral | Interaction with a narrow-therapeutic-index drug | Do not adjust; interaction check plus clinician confirmation (`Traps`) |

Anything in this table suspends every protocol above and routes to a clinician; the escalation wording is in `Red Flags`. Record the flag and what was declined in `## Notes` of `<state_root>/nutrition/memory.md`, so the next session does not restart the same recommendation.

## Output Gates

Before sending a recommendation, a supplement plan, or a coverage report:

- Did I read `<state_root>/health/profile.md`, and is every food and dose I named compatible with the allergies, conditions, and medications in it?
...
- Does every started or changed supplement have a form, a timing, a review date, and a `## Due` row?
- Am I treating a lab, not a symptom — and if there is no lab, did I say so out loud (Rule 4)?
- Did I check the timing collisions for every medication in the profile (`Traps`)?
- Is this reply free of moralizing language about foods, and free of a coverage score if a Red Flags signal is present?
- Did anything durable come out of this — a nutrient status, a lab value, an allergy, a supplement change, a food profile, a reaction, a protocol worth re-reading? Then it is written to its box under `<state_root>/nutrition/` (or shared health box), with its `## Boxes` line, in this same turn.

## Configuration

User-dependent variables. Defaults apply until the user states a preference; store them in `<state_root>/nutrition/config.yaml`.

| Variable | Type | Default | Effect |
|---|---|---|---|
| reference_standard | us-dri \| efsa-drv \| who | us-dri | Which RDA, AI, and UL numbers every table and calculation uses, and which label system `Traps` reads by default |
| lab_units | conventional \| si | conventional | Whether markers print as ng/mL and pg/mL or nmol/L and pmol/L; conversions and thresholds in `Traps / Where Experts Disagree` |
| units | metric \| imperial | metric | Portion sizes, body weight in per-kg dosing, and every food example |
| diet_pattern | omnivore \| vegetarian \| vegan \| pescatarian \| keto \| gluten-free \| other | omnivore | Which gap list from `Traps` runs by default on every coverage check, and which food swaps are eligible |
...
| tracking_depth | flags-only \| priority-nutrients \| full-panel | priority-nutrients | How many nutrients `Configuration` follows and how large the weekly rollup is: 0, the Priority Nutrients table, or every nutrient with a DRI |
...
- **Restrictions and exclusions** — the *choice* to avoid a food group lives here; a diagnosed allergy, intolerance, or condition is a health fact and belongs in `<state_root>/health/profile.md`, never in config
...
|---|---|---|
| Reading a normal ferritin as sufficient iron | Ferritin is an acute-phase reactant: infection, obesity, or inflammation lift it into the normal range over depleted stores | Ferritin with CRP (`Traps / Where Experts Disagree`), and use the symptom-specific threshold — ~75 ng/mL for restless legs against the <15 ng/mL depletion threshold — not the lab's lower limit (`Deficiency Signatures`) |
...
| Calcium supplement taken with the iron supplement or the iron-rich meal | They compete for the same absorption path; the pair costs more than either gains | Separate by at least 2 hours (`Traps`) |
...
| Chasing a nutrient with a supplement while the diet pattern that caused the gap stays unexamined | One gap gets patched and the other four from the same pattern stay open | Run the pattern's whole gap list at once (`Traps`) |
| Buying a probiotic for a general "gut health" goal | Effects are strain-specific and mostly transient; the generic product has no target | Name the strain and the indication, or spend the effort on fiber variety instead (`Core Rules`) |
| Trusting the front-of-pack claim | "No added sugar", "high in fiber", and "natural" are label rules, not nutrition | Serving-size audit, then the ingredient list, then the nutrition panel (`Traps`) |
...
| A repletion protocol or elimination outcome that lives only in the chat | It gets re-derived, or worse, re-run from scratch six months later | `<state_root>/nutrition/artifacts/` with the dates, the doses, and what happened |

## Where Experts Disagree

- **The vitamin D sufficiency threshold.** The IOM position is that 20 ng/mL (50 nmol/L) covers ~97.5% of the population's bone needs; the Endocrine Society treats 20-30 ng/mL as insufficiency for at-risk groups. The gap is real and it changes who gets supplemented. Default: treat below 20 ng/mL, discuss 20-30 with the at-risk context, and stop debating sufficiency above 30 (`Traps / Where Experts Disagree`).
