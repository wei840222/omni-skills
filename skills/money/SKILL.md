...
Most money questions are sequencing questions in costume: not "is this a good fund" but "is this where the next unit belongs". Name the step of the ladder, attach a number and a date, and say what it displaces. Work from defaults immediately: never open by asking for income, balances or goals — answer with the default and take whatever figures the user volunteers. The one exception to silence is `country`: while it is unset, state the jurisdiction you are assuming before quoting an account type, a tax rule or a benefit (Rule 3). That is a statement, not a question. Precedence for any value: `<state_root>/money/config.yaml` → `<state_root>/profile.yaml` (shared universals: currency, locale, country) → the Configuration table default. An observation never overwrites a declaration.
...
|---|---|---|
| "Should I pay this off or invest it?" | Compare after-tax rates on both sides; anything above `high_interest_rate_pct` is a guaranteed return nothing else matches | debt |
| Card at 20%+, only minimums going out | Minimum-payment trap maths, then payoff order and a rate-reduction call | debt |
| Score dropped, or a loan was refused | Utilization is a statement-date snapshot; report errors, then freeze mechanics | credit |
| How much cash to hold, and where | Months derived from volatility, not a slogan; instrument chosen by horizon | emergency fund |
| "Where does my money even go?" | Fixed / variable / sinking split, the savings-rate formula, the annual leak audit | budget |
| Raise, promotion, second income, equity comp | Split the raise before it lands; the income lever beats every optimization below it | income |
| Freelance, commission or seasonal income | Tax reserve per invoice, two-account separation, budgeting from the floor month | self employed |
| Allocation, fees, "is now a bad time to buy" | Horizon sets the asset, fee drag, rebalancing bands, lump sum versus averaging | investing |
| "Am I on track to stop working?" | 25× spending, withdrawal-rate honesty, sequence risk, which wrapper fills first | retirement |
| A tax question or a year-end decision | Marginal versus effective, account order, harvesting, benefit cliffs | taxes |
| Which cover, what deductible, what to drop | Insure the loss you cannot absorb; the deductible break-even | insurance |
| Rent or buy, remortgage, relocate | Round-trip transaction cost, break-even horizon, lender maths versus yours | housing |
| Car, wedding, renovation, "can I afford this?" | Total cost of ownership, the affordability test, the cooling-off rule | big purchases |
| Bonus, inheritance, vest or sale proceeds landed | Withhold the tax, park it, then walk the ladder in order | windfalls |
| Job loss, illness, divorce, a death | The first-72-hours sequence, then the first month; what never gets liquidated | shocks |
| A pitch, an adviser, a "guaranteed" return | Read the fee structure and the incentive before the product | scams |
| Partner, children or parents in the picture | Joint versus separate, dependants, beneficiaries, the money conversation | household |
| "How are we doing?", or a review is due | Monthly, quarterly and annual checklists; the net-worth snapshot | reviews |
...
2. **Real, after tax, or it is not a number.** Real return = (1 + nominal) ÷ (1 + inflation) − 1: 7% nominal at 3% inflation is 3.88% real, not 4%, and over 30 years that gap is a third of the balance. Compare a debt rate to an investment return only after both are on an after-tax footing (taxes).
...
5. **Horizon picks the asset, mood does not.** Under 2 years → cash or short government bills; 2-5 years → short-duration bonds; over 7 years → equities dominant. Broad-market drawdowns have taken five years and more to recover in real terms, so money with a date inside that window is not invested money (investing).
...
7. **Insure the loss that would break you; self-insure the rest.** Cover what cannot be absorbed by `emergency_fund_months` of buffer, and buy the highest deductible that buffer covers. Break-even = (premium saved per year × expected years) versus (deductible increase) (insurance).
...
9. **The plan lives in a file, not in the reply.** A payoff order, a target, a rate or a decision that exists only in a chat is gone next session — write it to the destination `references/persistence.md` defines, in the same turn it is agreed.
...
| 3 | Clear every balance priced above `high_interest_rate_pct` | Nothing left above the line (debt) |
| 4 | Fill the buffer to `emergency_fund_months` | Months of **core spending**, never of gross income (emergency fund) |
| 5 | Long-term money into the most tax-advantaged wrapper available | Saving at `savings_rate_target_pct` of gross (investing, retirement) |
...
| Minimum-payment horizon | A card at 22% APR pays 1.83% of the balance in interest monthly, so a 2%-of-balance minimum retires 0.17% of principal; typical payoff runs past 15 years and repays roughly twice the principal | The minimum is engineered to be affordable, not to end (debt) |
| Affordability | (monthly finance + running cost) ÷ net monthly income | The price is the deposit on the running cost: insurance, fuel, maintenance, tax, replacement (big purchases) |
...
| Safe withdrawal | 4% of the starting portfolio, then inflation-adjusted (Bengen) | Derived from US 30-year rolling periods with a 50-75% equity mix and no fees; subtract your fee and widen for a longer or non-US retirement (retirement) |
...
|---|---|---|
| A housing or utility payment missed, or a card used to pay a card | Cashflow insolvency, not a budgeting problem | Stop optimizing; run the crisis sequence in shocks and name the free statutory or non-profit debt-advice route for their `country` |
...
| Someone is pressuring a decision inside 24-72 hours | Fraud pattern | scams; no legitimate opportunity expires over a weekend |
| An older person's accounts newly controlled by a "friend", carer or adviser; unexplained transfers | Financial abuse | Freeze and limit steps in scams, then the local adult-safeguarding route |
...
| emergency_fund_months | number (1-24) | 6 | Buffer target in emergency fund and the gate on ladder step 4 |
| high_interest_rate_pct | number (%) | 8 | The line separating "clear it first" from "invest instead" at ladder step 3 (debt) |
| savings_rate_target_pct | number (0-70 %) | 20 | Target in budget, the split applied to a raise in income, and the input to the independence estimate |
| risk_posture | conservative \| balanced \| aggressive | balanced | Default equity share and rebalancing bands in investing; breaks ties when two options are close |
| household | single \| couple \| family | single | Whether guidance covers joint accounts, dependants, beneficiaries and survivor cover (household, insurance) |
| review_day | number (1-28) | 1 | Day of month the cashflow review falls due in the `## Due` table (reviews) |
...
- **Tracking tooling** — spreadsheet, an app, bank exports or nothing at all — affects what reviews asks for and what format a plan is delivered in
...
| Treating a fixed mortgage rate as its headline cost | After tax and after inflation, a fixed 3% loan during 4% inflation is being repaid in cheaper money each year | Compare the after-tax real rate against the after-tax real alternative (housing) |
...
| Telling someone mid-crisis to build a six-month fund | Impossible advice ends the conversation and the plan | A crisis has its own sequence: stabilize cashflow, protect housing, then rebuild (shocks) |
...
| A budget with no sinking funds | Annual and irregular costs then arrive as "emergencies" and consume the buffer built for real ones | Every known irregular cost gets a monthly twelfth in budget |
| Advising a couple as if they were one person | Two incomes, two risk postures, two credit files, and often two sets of dependants | Set `household` and use household before allocating joint money |
...
- **Avalanche versus snowball.** Highest rate first is arithmetically optimal; smallest balance first wins on completion rates for people who have abandoned a plan before. The gap is usually small in absolute money on a mixed balance sheet, so the tie-break is the user's history, not the spreadsheet (debt).
...
- **Buying a home as a financial decision.** Owner-occupied housing is consumption bought with leverage, not an investment; the honest defence is forced saving, security of tenure and inflation-linked housing costs, none of which appears in a return calculation (housing).
- **Paying for advice.** Percentage-of-assets pricing scales the fee with the balance while the work does not; flat-fee and hourly advisers charge less over a lifetime but require the client to implement. What is not disputed: commission-based product sales and advice are different jobs (scams).
