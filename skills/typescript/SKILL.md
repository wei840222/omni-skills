...
metadata:
  clawdbot:
    ...
    configPaths:
    - <state_root>/typescript/
    - ~/typescript/
    - ~/clawic/typescript/
  related-skills: '{"javascript": "runtime semantics: coercion, closures, event loop", "react": "typing components, hooks, and props in practice", "nodejs": "Node runtime, packaging, and ESM/CJS beyond the type layer", "deno": "TypeScript-native runtime without a build step"}'
  openclaw: '{"requires":{"config":["<state_root>/typescript/","~/typescript/","~/clawic/typescript/"]}}'
---

User preferences and memory live in `<state_root>/typescript/` (see `references/setup.md` on first use, `references/memory-template.md` for the file format). If you have data at an old location (`~/typescript/` or `~/clawic/typescript/`), move it to `<state_root>/typescript/`, and say in one line that you moved it and from where.
...
- For runtime JavaScript semantics (closures, event loop, coercion) — that is the javascript skill

## Quick Reference

| Situation | File to load | How to apply |
|---|---|---|
| Value of unknown shape (API, `JSON.parse`, `catch`) | N/A | `unknown` + narrow before use; always use `unknown` |
| Config/lookup object losing literal types | N/A | `satisfies` (TS >=4.9), not a type annotation |
| Union variant not narrowing | N/A | Add a literal discriminant field + exhaustive `never` check |
| 40-line "not assignable" error | `references/errors.md` | Load to decode errors and fix root causes. |
| Cryptic diagnostic (TS2589, TS2742, "no overload matches"), error only in CI | `references/errors.md` | Load to decode errors and fix root causes. |
| Generic won't infer / variance confusion / overload design | `references/generics.md` | Load for generics and constraints. |
| `Partial`/`Omit`/`Record` behaving oddly | `references/utility-types.md` | Load to understand TS primitive behavior. |
| Untyped npm package, globals, augmentation, publishing types | `references/declarations.md` | Load for .d.ts files and global augmentations. |
| JS conversion, strict-flag rollout, `as` debt, TS version upgrade | `references/migration.md` | Load for TS migrations and strictness rollouts. |
| target/module/paths choices, monorepo, project references | `references/tsconfig.md` | Load for config strategy. |
| Green compile, runtime crash (`x is not a function`, `ERR_REQUIRE_ESM`) | `references/modules.md` | Load for imports and transpile bounds. |
| External data crossing a trust boundary | `references/boundaries.md` | Load for validation and API borders. |
| tsc slow, editor lag, compiler out of memory | `references/performance.md` | Load for slow TS compilation issues. |
| Anything else typed | N/A | Default: full `strict`, infer locals, annotate exports |

Depth on demand: `references/errors.md` diagnostic decoding · `references/generics.md` inference, variance, conditional types, overloads · `references/utility-types.md` built-in type traps · `references/declarations.md` .d.ts, augmentation, publishing · `references/migration.md` JS-to-TS, strictness ratchets, TS upgrades · `references/tsconfig.md` compiler configuration · `references/modules.md` ESM/CJS interop · `references/boundaries.md` runtime validation · `references/performance.md` compile and editor speed.

## Core Rules

1. **Full `strict` or you are reviewing half the code.** Every flag off is a bug class the checker won't report. Greenfield: `strict: true` day one plus `noUncheckedIndexedAccess` (TS >=4.1). Legacy: stage flags with a ratchet (`references/migration.md`).
2. **`unknown` in, typed out.** Data you didn't construct (network, JSON, env, storage) enters as `unknown` and gets validated once at the boundary (`references/boundaries.md`); past that point no `any` and no re-checking.
...
7. **Errors read bottom-up.** The first lines of a long diagnostic are wrappers; the root cause is the deepest `Types of property ... are incompatible` line (`references/errors.md`).
...
- `verbatimModuleSyntax` (TS >=5.0) — forces `import type` for type-only imports; the modern replacement for `isolatedModules` import hygiene (`references/modules.md`)

Everything else about compiler configuration — target/lib, `module` × `moduleResolution`, paths, monorepos — lives in `references/tsconfig.md`; import/export mechanics and transpile-only compatibility in `references/modules.md`.
...
- Slow type checking has structural causes and a measurement workflow — `references/performance.md`
...
User-dependent variables. Defaults apply until the user states a preference; store them in `<state_root>/typescript/config.yaml`.

| Variable | Type | Default | Effect |
|---|---|---|---|
| runtime_target | node \| browser \| bun \| deno \| isomorphic | node | Drives `module`/`moduleResolution` and lib advice in `references/tsconfig.md` and import-extension rules in `references/modules.md` |
...
| compile_pipeline | tsc \| transpile-only | transpile-only | transpile-only (esbuild, swc, Babel, bundlers) enforces the single-file-safe subset: no `const enum`, `isolatedModules` rules apply (`references/modules.md`); tsc lifts those bans |
| validation_library | zod \| valibot \| arktype \| ajv \| none | none | Names the schema library used in boundary-validation examples (`references/boundaries.md`); none keeps guidance library-neutral |
...
|---|---|---|
| `enum` | The one TS feature with runtime emit; `const enum` breaks under transpile-only compilers | `as const` object + `keyof typeof` (`references/modules.md`) |
...
| Trusting excess property checks as validation | They fire only on fresh object literals — a value passed through a variable skips them | Runtime validation at boundaries (`references/boundaries.md`) |
| `// @ts-ignore` | Keeps suppressing after the error is fixed, hiding the next one | `// @ts-expect-error` (TS >=3.9) — errors itself once stale |
| `!` to silence `strictNullChecks` | Each one is an invisible cast and a crash candidate | Narrow, early-return, or `??`; sanctioned only for framework-assigned class fields (`references/migration.md`) |
...
- **Runtime validation depth.** Edges-only is the default — validating internally recomputes what the checker already proved. The schema-first school (schema as source of truth, static types inferred from it) wins when one shape crosses many boundaries (`references/boundaries.md`).
