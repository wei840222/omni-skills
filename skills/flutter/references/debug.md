...
3. **Reproduce it in a widget test.** A failing `testWidgets` that pumps the offending widget turns an intermittent app-level bug into a one-second loop, and leaves a regression test behind (`references/testing.md`).
...
| `Looking up a deactivated widget's ancestor` | A `BuildContext` captured before an await, used after | Capture the `NavigatorState`/`ScaffoldMessengerState` BEFORE awaiting (`references/async.md`) |
| `A RenderFlex overflowed by N pixels` | A child wants more than the axis allows | `Expanded`/`Flexible`, or make the axis scrollable (`references/layout.md`) |
| `Vertical viewport was given unbounded height` | A scrollable inside a `Column` or another scrollable | `Expanded`, a fixed height, or slivers (`references/layout.md`) |
| `Incorrect use of ParentDataWidget` | `Expanded`/`Positioned` is not a direct child of its flex/stack | Remove the widget in between (`references/layout.md`) |
| `Multiple widgets used the same GlobalKey` | One key on two live widgets, or a key recreated on rebuild | Make the key a field of the `State`, or stop using a `GlobalKey` (`references/widgets.md`) |
| `Bad state: Stream has already been listened to` | A single-subscription stream re-listened after a rebuild | Cache the stream in a field, or make it broadcast (`references/async.md`) |
| `A Ticker was started and is still running` | An `AnimationController` was never disposed | Dispose it (SKILL.md rule 3) |
| `MissingPluginException` | The plugin was added without a full restart, or this platform has no implementation | Fully restart the app; for a project that still uses CocoaPods, run its established `pod install` workflow (`references/platform.md`) |
| `PlatformException(code, message)` | Native code answered with a failure | Branch on `code`; read the native log for the real reason (`references/platform.md`) |
| `type 'Null' is not a subtype of type 'String'` | JSON parsed with an unchecked cast | Parse defensively at the boundary (`references/data.md`) |
| `Scaffold.of() called with a context that does not contain a Scaffold` | The context is above the Scaffold it created | `Builder`, an extracted widget, or `ScaffoldMessenger.of` (`references/widgets.md`) |
| `Unable to load asset` | Undeclared in `pubspec.yaml`, or a path typo | Declare it; directory entries need a trailing slash (`references/release.md`) |
...
1. Is the widget hit-testable? Painting outside the parent's bounds is visible but not tappable (`references/layout.md`).
...
3. Transparent area under a `GestureDetector`: needs `behavior: HitTestBehavior.opaque` (`references/widgets.md`).
4. Is the callback null? A `disabled` button (`onPressed: null`) looks nearly identical to an enabled one in many themes.
5. Is the handler running but the UI not changing? Add a print, then check for a mutation without `setState` (`references/state.md`) — the silent no-op.
...
- Adding or removing a PLUGIN requires a full restart: the native side registers at startup (`references/platform.md`).
...
- If a restart also does nothing, the artifact is stale: `flutter clean` earns its keep exactly here and nowhere else (`references/commands.md`).
...
2. Which thread missed the frame? UI vs raster changes the entire diagnosis (`references/performance.md`).
...
4. Frozen rather than janky (no frames at all): a synchronous block on the UI isolate — a big `jsonDecode`, a file read, a sort. Move it to an isolate (`references/async.md`).
5. Jank only while scrolling images: decoded image size (`references/performance.md`).
6. Jank only the first time an effect plays: shader compilation on the pre-Impeller renderer — confirm which renderer started before treating it as a code problem (`references/performance.md`).
...
3. Rule out intentional retention first: `AutomaticKeepAliveClientMixin`, `IndexedStack`, and a large `cacheExtent` all hold memory by design (`references/state.md`).
4. Image memory dominates most apps: check the image cache before hunting objects (`references/performance.md`).
...
| Difference | Check |
|---|---|
| Icons blank, assets missing | Tree-shaking and `pubspec.yaml` declarations (`references/release.md`) |
| Logic inside an `assert` | `assert`s are stripped in release |
| Reflection or `Type.toString()` | Obfuscation renamed it (`references/release.md`) |
...
| Unreadable crash stack | Symbolize with the symbols from THAT build (`references/release.md`) |

## Works on My Machine, Fails in CI

| Difference | Check |
|---|---|
| SDK version | Pin the Flutter version in CI to the team's (`references/dependencies.md`) |
| Golden mismatch | Goldens are platform-specific; generate on the CI image (`references/testing.md`) |
| Generated code | `build_runner` not run, or a stale committed output (`references/dependencies.md`) |
| Test surface size | CI runs the default 800 × 600; a size-dependent test needs an explicit view size (`references/testing.md`) |
| Test hangs then times out | `pumpAndSettle` against a repeating animation (`references/animations.md`) |
| A real network call | A test hitting the network flakes on a runner; block it in `setUpAll` (`references/testing.md`) |
...
- Version solving failed: `references/dependencies.md`.
