...
- A context captured in a closure and used after an `await` may be dead: "Looking up a deactivated widget's ancestor". Guard with `mounted`, or capture the object you actually need before the await (`references/async.md`).
...
| Repeat a widget over data | `...items.map(...)` spread, or `.builder` for long lists | Spreading builds all of them (`references/layout.md`) |
...
- Material 3 is the default for `ThemeData` on current SDKs; a design that looks suddenly different after an SDK upgrade is usually this, not a regression in your code (`references/dependencies.md`).
- Test both brightnesses. A widget test can pump with `ThemeData.dark()` in a loop over both (`references/testing.md`).

## GlobalKey: When It Is Actually Right

| Use | Verdict |
|---|---|
| `GlobalKey<FormState>` to validate a form | Right — no other access path exists (`references/forms.md`) |
| A `navigatorKey` for navigation without context | Defensible for a few entry points (`references/navigation.md`, Where Experts Disagree in SKILL.md) |
...
| Reaching a child's `State` to call a method on it | Wrong — pass a callback down or share a notifier (`references/architecture.md`) |
...
- Painting outside the parent's bounds does not extend hit testing: a button half outside a `Stack` is only tappable in the visible-and-inside half (`references/layout.md`).
