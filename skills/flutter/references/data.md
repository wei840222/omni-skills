...
The data layer owns three responsibilities and should own nothing else: talk to the network, keep a local copy, and hand the rest of the app domain objects that never mention JSON (`references/architecture.md`).
...
- Parse large payloads off the main isolate (SKILL.md rule 6): `Isolate.run(() => parseUsers(body))` where the function takes the raw STRING and returns the model list, so both the decode and the mapping move (`references/async.md`). Passing an already-decoded `Map` in defeats the purpose — the expensive part already ran on the UI thread.
...
| Interceptors, cancellation, upload progress, retries | `dio` | `CancelToken` is the only clean cancellation path in Flutter (`references/async.md`) |
...
- `shared_preferences` reads the whole store into memory on first access — fine for dozens of small keys, wrong for a cache. Its writes are async and can be lost if the process dies immediately after (`references/state.md`, app lifecycle: persist on `paused`).
...
- Secrets compiled into the app are readable: an API key in Dart source is extractable from the release binary regardless of obfuscation (`references/release.md`). Anything that must stay secret lives behind your own backend.
- Log request bodies only in debug builds, and never log tokens: `assert` blocks are stripped in release and are the cheapest way to guarantee that (`references/release.md`).
