...
- A field inside a `ListView.builder` gets recycled: without a stable key, scrolling away and back moves the text to a different row (`references/state.md` rule 4).
...
- `FocusScope.of(context).unfocus()` dismisses the keyboard. Tap-outside-to-dismiss needs a `GestureDetector` with `behavior: HitTestBehavior.opaque` around the body (`references/widgets.md`).
- Focus traversal follows the widget tree, not the visual layout. A two-column form built as two `Column`s tabs down one column then the other; `FocusTraversalGroup` with an ordering policy fixes it — and this matters on web and desktop where Tab is the primary navigation (`references/adaptive.md`).
...
- `Scaffold.resizeToAvoidBottomInset` defaults to `true`: the body shrinks by the keyboard's height, which is why a `Column` that fit suddenly overflows (`references/layout.md`). Wrap the form body in a `SingleChildScrollView` — the standard answer.
...
- `MediaQuery.viewInsetsOf(context).bottom` is the keyboard height; use it for a footer button that must sit above the keyboard. `viewPadding` and `padding` are different things (`references/adaptive.md`).
...
- SMS one-time codes: `AutofillHints.oneTimeCode` gets the iOS keyboard suggestion and Android autofill for free — cheaper and more reliable than reading SMS, which requires permissions and store justification (`references/platform.md`).
...
- After success, decide explicitly: pop with a result (`references/navigation.md`), reset the form (`_formKey.currentState!.reset()`), or navigate onward. Doing nothing leaves a submitted form on screen that the user will submit again.
