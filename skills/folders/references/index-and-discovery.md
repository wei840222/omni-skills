...
Store the index only at `<state_root>/folder-index.json`. If multiple candidate roots exist, tell the user which root was selected and keep other copies unchanged. Perform any migration, merge, or legacy-file removal only when the user explicitly requests it.
